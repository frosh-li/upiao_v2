...
class StationService extends Service {
    ...
    async parserStation(data) {
        ...
        await app.redis.setex(`realtime:station:${data.sn_key}`, 300, JSON.stringify(obj));
    ...
    async parserGroup(data, sid) {
        ...
        await app.redis.setex(`realtime:group:${bulkData[0]['sn_key']}`, 300, JSON.stringify(bulkData));
        await this.app.mongo.db.collection('group_data').insertMany(bulkData);
        logger.info("组数据批量写入完成", bulkData[0]['sn_key']);
    ...
    async parserBattery(data, sid) {
        ...
        await app.redis.setex(`realtime:battery:${bulkData[0]['sn_key']}`, 300, JSON.stringify(bulkData));
        await this.app.mongo.db.collection('battery_data').insertMany(bulkData);
        logger.info("电池数据批量写入完成", bulkData[0]['sn_key']);
    }
