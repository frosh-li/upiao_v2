    ...
    async parser(data) {
        ...
        if(data.GroupErr) {
            ...
            GroupErrs.forEach(GroupErr => {
                errorBulks = errorBulks.concat(this.buildBulk(GroupErr.sn_key, record_time, 'group', GroupErr));
            })
        }
        ...
        if(data.BatteryErr) {
            ...
            BatteryErrs.forEach(BatteryErr => {
                errorBulks = errorBulks.concat(this.buildBulk(BatteryErr.sn_key, record_time, 'battery', BatteryErr));
            })
        }
        ...
        await this.app.redis.setex(`caution:${sn_key}`,60, JSON.stringify(errorBulks))
    }
}
