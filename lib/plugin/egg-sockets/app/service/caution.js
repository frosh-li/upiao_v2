...
class CautionService extends Service {
    ...
    async parser(data) {
        ...
        let record_time = moment().utcOffset(-8).format('YYYY-MM-DD HH:mm:ss');
        let errorBulks = [];
        let sn_key = '';
        if(data.StationErr) {
        ...
        let insertQuerys = [MyAlerts.bulkCreate(errorBulks)];
        logger.info(`${sn_key}当前时间${now}:${lastHistory}`)
        if(now - lastHistory > app.config.sockets.client.insertCautionInterval){
            insertHistory = true;
            logger.info(`${sn_key}需要插入历史`);
            insertQuerys.push(MyAlertsHistory.bulkCreate(errorBulks));
            app.updateLastCautionBySnKey(sn_key);
        }

        // 先清理之前的报警数据，并插入最新的报警数据
        await this.app.model.query(`delete from my_alerts where floor(sn_key/10000) = ${Math.floor(sn_key/10000)}`);
